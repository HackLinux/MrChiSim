...
class ChibiJNI {
  // --- Native Send Methods
  @native def hexSend(hex : String, name : String) : Int

  // --- Native Read Methods
  @native def hexRead(name : String) : String
...
def doTesting() {
  ...
  var i = 0
  for(z <- 1 to 100) {
    val filename = Random.nextInt().abs.toString.takeRight(8)
    ...
    chi.hexSend(filename + " " + toHex("OMGHAXLOL"), filename)
    val res = chi.hexRead(filename)
